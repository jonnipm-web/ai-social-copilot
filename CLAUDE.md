...
- **Sempre apresentar relatórios, documentação e saídas longas dentro de um bloco de código** (caixa de texto), usando a sintaxe ` ```text ` … ` ``` `. Nunca soltar texto longo diretamente no chat sem estar dentro de uma caixa de texto.
- **Sempre apresentar primeiro a alternativa mais fácil e prática** para qualquer processo que o usuário precise executar (ex: um workflow com um clique no GitHub Actions, um script, uma ferramenta automatizada). Só depois apresentar opções mais complexas. Todas as instruções de processo devem usar linguagem didática para leigo: explicar o que é cada ferramenta, por que está sendo usada e o que esperar como resultado.
