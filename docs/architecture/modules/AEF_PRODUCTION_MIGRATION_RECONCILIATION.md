...
- Each migration applies only on top of its **real** dependencies (Codex G2V-03
  corrected an overclaim; this is not the file order). Every Lab migration opens
  with a precondition guard (`AEF_PRECONDITION` or `LAB_PRECONDITION`, `AE010`)
  that fails **before creating anything** when one is missing.
  - `20260925` does not need `20260923` and may be applied without it.
  - `20260926` refuses to apply without `20260923` **and** the complete
    `20260925`.
  - `20260927` needs only `20260925`.

  This is tested in the runner as `AEF_FAIL_FAST: PASS`. A failed apply is
  checked against a **whole-schema catalog fingerprint**, which must be
  byte-identical before and after (Codex G2V-04): every relation, column,
  constraint, trigger, policy, function body and ACL in `public`.
...
- **Atomicity** (Codex G2-05): the runbook requires each file to be applied in a
  ...
  objects were created) leaves the whole-schema catalog fingerprint unchanged.
- **Look-alike dependency** (Codex G2V-02): `20260926` also requires
  `subject_roles` to carry exactly the primary key, role CHECK and RLS that
  `20260923` creates. A table with only the right column names is refused. This
  is tested.
...
After the Codex verification round the preflight also:
...
- compares a **structural fingerprint** for every installed chain migration
  ...
  - `subject_roles`;
  - the part of `business_memory` that `20260924` owns;
  - all `aef_*` tables and `aef*` functions, whose expected value depends on the
    installed state (persistence only, or persistence + hardening).

  Extra columns, stray `aef_` tables, altered constraints, defaults, policies or
  function bodies all FAIL. The expected values are generated by
...
code page is WIN1252 double-encodes those characters and would (correctly) be
reported as drift.
