...
| PDF: `%PDF-` within the first 1024 bytes | `FILE_SIGNATURE_INVALID` |
| DOCX/XLSX: valid zip + `[Content_Types].xml` + main part; `vbaProject.bin` / `macroEnabled` / `.docm`/`.xlsm` refused | `UNSUPPORTED_FILE_TYPE` / `FILE_SIGNATURE_INVALID` |
...
| Parser | Bounds |
|---|---|
| ZIP | EOCD search bounded; ZIP64 refused; ≤ 2000 entries; duplicate names refused; encrypted entries refused; only stored/deflate; per-part size cap; **shared inflate budget 32 MB per file**; streaming inflate aborts at the limit |
| PDF | ≤ 500 pages, ≤ 50 000 objects; page tree depth 64, cycle-safe; FlateDecode only (others noted, skipped); `/Encrypt` → FAILED ENCRYPTED; image-only → OCR_REQUIRED |
| DOCX | tag scan (no XML entity expansion — only the 5 predefined entities + numeric references are decoded) |
| XLSX | ≤ 20 sheets, ≤ 200 000 cells; external relationship targets and `..` refused; formulas kept as text, **never evaluated** |
...
| All | ≤ 250 000 segments; an extractor exception → `FAILED EXTRACTOR_ERROR` (never SUCCESS) |
