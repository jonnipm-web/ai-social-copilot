...
| URLs | parsed | **origin only** (scheme + host; never userinfo, port, path, query, fragment) and only for institutional types (OFFICIAL_REGISTRY, GOVERNMENT_RECORD, REGULATOR, COURT_RECORD) or a host within the subject's declared domains; every other URL withheld (Codex I6G1R-06), with a visible `EXCERPT_WITHHELD` / SOURCE limitation (Codex I6G3-01) | none known |
...
`privacy_test.ts` PV-01..33 (structured ids incl. Unicode / labelled / MRZ,
