...
class IntelligenceDebugService {
  ...
  ValidationReport runValidation({
    ...
    tests.add(ValidationTest(
      ...
      description:
          'Todos os projetos possuem ações ou oportunidades cadastradas?',
      status: _status(projectsWithAssets, projects.length),
      passed: projectsWithAssets,
      total: projects.length,
      failedItems: projects
          .where((p) =>
              !actions.any((a) => a.projectId == p.id) &&
              !labItems.any((l) => l.projectId == p.id))
          .map((p) => p.name)
          .toList(),
      suggestion:
          'Adicione ações ou oportunidades no Opportunity Lab para cada projeto.',
    ));
    ...
    tests.add(ValidationTest(
      ...
      description:
          'Todos os ativos (ações e oportunidades) estão vinculados a um projeto?',
      ...
      failedItems: [
        ...actions
            ...
            .map((a) => 'Ação: ${a.title}'),
        ...labItems
            .where((l) => l.projectId == null)
            .map((l) => 'Oportunidade: ${l.title}'),
      ],
      suggestion:
          'Vincule todas as ações e oportunidades a projetos existentes.',
    ));

    // T03 — Documentos indexados
    final indexedDocs =
        knowledgeItems.where((k) => k.status == 'analyzed').length;
    tests.add(ValidationTest(
      id: 'T03',
      name: 'Documentos Indexados',
      description:
          'Todos os documentos foram processados pelo sistema de conhecimento?',
      ...
      failedItems: knowledgeItems
          ...
          .toList(),
      suggestion:
          'Acesse o Cofre de Conhecimento e reprocesse os documentos pendentes.',
    ));

    // T04 — Personas com aprendizado
    final personasWithLearning =
        learningProfiles.where((p) => p.learningScore > 0).length;
    tests.add(ValidationTest(
      id: 'T04',
      name: 'Personas com Aprendizado',
      description:
          'Todas as personas possuem pelo menos um treinamento registrado?',
      ...
      failedItems: learningProfiles
          ...
          .toList(),
      suggestion:
          'Treine as personas no Cofre de Conhecimento com documentos relevantes.',
    ));
    ...
    tests.add(ValidationTest(
      ...
      description:
          'Todas as recomendações possuem dados rastreáveis e confiança ≥ 40%?',
      ...
      failedItems: recommendations
          ...
          .toList(),
      suggestion:
          'Adicione análises de mercado e métricas ROI para aumentar a confiança.',
    ));
    ...
    tests.add(ValidationTest(
      ...
      description:
          'Todos os scores do ecossistema possuem dados reais suficientes?',
      ...
      failedItems: scores
          ...
          .toList(),
      suggestion:
          'Vincule análises de mercado e registre métricas ROI para melhorar os scores.',
    ));

    // ── Phase 10I Tests ────────────────────────────────────────────────────

    // T07 — Market Profiles válidos
    final profilesWithData =
        marketProfiles.where((p) => p.confidence >= 50).length;
    tests.add(ValidationTest(
      id: 'T07',
      name: 'Market Profiles Válidos',
      description:
          'Todos os projetos possuem Market Profile com confiança ≥ 50%?',
      ...
      failedItems: marketProfiles
          ...
          .map((p) =>
              '${p.projectName} [${p.confidence}% confiança — fonte: ${p.profileSource}]')
          .toList(),
      suggestion:
          'Vincule análises de mercado para elevar a confiança do Market Profile.',
    ));

    // T08 — Revenue Projections válidas
    final revenueWithPlan =
        revenueIntelligence.where((r) => r.hasRealPlan).length;
    tests.add(ValidationTest(
      id: 'T08',
      ...
      failedItems: revenueIntelligence
          ...
          .toList(),
      suggestion:
          'Execute o Knowledge → Action Engine para gerar planos de receita automaticamente.',
    ));
    ...
    tests.add(ValidationTest(
      ...
      failedItems: scores
          ...
          .toList(),
      suggestion:
          'Gere planos de receita via Knowledge → Action Engine ou registre métricas ROI.',
    ));
    ...
    tests.add(ValidationTest(
      ...
      failedItems: scores
          ...
          .toList(),
      suggestion:
          'Vincule análises de mercado ou gere oportunidades via Knowledge → Action Engine.',
    ));

    // T11 — Decision coerente (sem ANÁLISE INCOMPLETA em projetos com dados)
    final incoherentDecisions = scores
        .where(
            (s) => s.recommendation == 'ANÁLISE INCOMPLETA' && s.hasEnoughData)
        .length;
    ...
    tests.add(ValidationTest(
      ...
      description:
          'Nenhum projeto com dados recebe recomendação "ANÁLISE INCOMPLETA"?',
      status: _status(coherentDecisions, scores.length),
      passed: coherentDecisions,
      total: scores.length,
      failedItems: scores
          .where((s) =>
              s.recommendation == 'ANÁLISE INCOMPLETA' && s.hasEnoughData)
          .map((s) =>
              '${s.project.name} [tem dados mas recebeu ANÁLISE INCOMPLETA]')
          .toList(),
      suggestion:
          'Verifique o Motor de Decisão — projetos com dados não devem receber ANÁLISE INCOMPLETA.',
    ));
    ...
    tests.add(ValidationTest(
      ...
      failedItems: scores
          .where((s) => s.ecosystemScore >= 40 && s.recommendation == 'PAUSAR')
          .map(
              (s) => '${s.project.name} [score=${s.ecosystemScore} mas PAUSAR]')
          .toList(),
      suggestion:
          'Motor de decisão com thresholds incorretos — verifique o EcosystemIntelligenceService.',
    ));
  ...
  ScoreBreakdown _buildBreakdown(
    ...
    if (analysis != null) {
      ...
      final timePart =
          (p.timeToRevenueDays >= 1 && p.timeToRevenueDays <= 90) ? 10.0 : 0.0;
      oppRaw = (revPart + priPart + timePart).round().clamp(0, 100);
      oppFormula =
          'min(50, revenuePotential/2000) + priorityScore×0.30 + (days≤90 ? 10 : 0)';
      oppSources.add(
          'revenuePotential=${p.revenuePotential} → ${revPart.round()}pts');
      oppSources.add(
          'priorityScore=${p.priorityScore} × 0.30 = ${priPart.round()}pts');
      if (timePart > 0) {
        oppSources
            .add('timeToRevenueDays=${p.timeToRevenueDays} (≤90d) → +10pts');
      } else {
        oppSources
            .add('timeToRevenueDays=${p.timeToRevenueDays} (>90d) → +0pts');
      }
      oppSources.add(
          'SEM análise de mercado vinculada — score derivado dos campos do projeto');
    }

    // ── Strategic Fit ────────────────────────────────────────────────────────
    final completedActions =
        pActions.where((a) => a.status == 'completed').length;
    ...
    final execComp = pActions.isEmpty
        ? 5.0
        : (completedActions / pActions.length * 100) * 0.20;
    final fitRaw =
        (marketComp + priorityComp + roiComp + execComp).round().clamp(0, 100);
    final fitSources = [
    ...
    if (pRoi.isNotEmpty) {
      ...
      roiSources.add(
          'total = R\$${totalRoiValue.round()} / 2000 × 100 = ${roiRaw}pts');
      ...
      roiSources
          .add('SEM métricas ROI reais — usando plano de receita como proxy');
      roiSources.add(
          'monthlyModerate = R\$${plan.monthlyModerate.round()} → ${roiRaw}pts');
    } else {
      roiRaw = 0;
    ...
    // ── Momentum ─────────────────────────────────────────────────────────────
    final recentActions =
        pActions.where((a) => a.createdAt.isAfter(cut30)).length;
    ...
    final momSources = [
      baseline > 0
          ? 'itens existentes → baseline ${baseline}pts'
          : 'sem itens → baseline 0pts',
    ...
    if (analysis != null)
      allSources.add(
          'MarketAnalysis: ${analysis.id.substring(0, 8)}... (${analysis.input.substring(0, math.min(30, analysis.input.length))}...)');
    if (plan != null)
      allSources.add(
          'RevenuePlan: ${plan.projectName} — R\$${plan.monthlyModerate.round()}/mês');
    if (pRoi.isNotEmpty) allSources.add('${pRoi.length} RoiMetric(s)');
    if (pActions.isNotEmpty)
      allSources
          .add('${pActions.length} Action(s) (${completedCount} concluídas)');
    ...
    return ScoreBreakdown(
      ...
      strategicFit: ScoreComponent(
        ...
        explanation:
            'Mede alinhamento estratégico: oportunidade de mercado, priorização, retorno esperado e capacidade de execução.',
      ...
      synergy: ScoreComponent(
        ...
        weight: 0.20,
        formula:
            'análise(+25) + lab×8(max30) + aprovados×10(max20) + ações×3(max15)',
        explanation:
            'Mede a riqueza do ecossistema ao redor do projeto: análise vinculada, oportunidades mapeadas e ações definidas.',
      ...
      momentum: ScoreComponent(
        ...
        explanation:
            'Mede a velocidade de atividade recente. Projetos inativos nos últimos 30 dias perdem momentum.',
