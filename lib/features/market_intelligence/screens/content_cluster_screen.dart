...
class ContentClusterScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ContentClusterScreen> createState() =>
      _ContentClusterScreenState();
}
...
class _ContentClusterScreenState extends ConsumerState<ContentClusterScreen> {
  ...
  Future<void> _build() async {
    ...
    if (kw.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Digite a palavra-chave principal'),
    ...
    try {
      ...
      await ref.read(marketAnalysisServiceProvider).buildContentCluster(
          widget.analysisId, analysis.input, kw,
          context: ctx);
  ...
  Widget build(BuildContext context) {
    final asyncCluster =
        ref.watch(contentClusterByAnalysisProvider(widget.analysisId));

    return Scaffold(
      backgroundColor: const Color(0xFF0F0F1A),
      appBar: AppBar(
        backgroundColor: const Color(0xFF0F0F1A),
        title: const Text('Content Cluster Engine',
      ...
      body: asyncCluster.when(
        loading: () => const Center(
            child: CircularProgressIndicator(color: Color(0xFFAB83FF))),
        error: (e, _) => Center(
            child: Text('Erro: $e',
            ...
            ? SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    const Text('Nenhum cluster ainda',
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        focusedBorder: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(12),
                          borderSide:
                              const BorderSide(color: Color(0xFFAB83FF)),
                        ),
                      ),
                    ),
                    const SizedBox(height: 16),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                            _running ? 'Gerando...' : 'Gerar Content Cluster'),
                      ...
                      child: Row(
                        children: [
                          const Icon(Icons.key_rounded,
                              color: Color(0xFFAB83FF), size: 18),
        ...
        ...roadmap.map(
          (item) => Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(10),
              border:
                  Border.all(color: const Color(0xFFFFD93D).withOpacity(0.15)),
            ),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
