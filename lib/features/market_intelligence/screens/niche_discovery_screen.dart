...
class NicheDiscoveryScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<NicheDiscoveryScreen> createState() =>
      _NicheDiscoveryScreenState();
}
...
class _NicheDiscoveryScreenState extends ConsumerState<NicheDiscoveryScreen> {
  ...
  Future<void> _discover() async {
    ...
    try {
      ...
      await ref
          ...
          .discoverNiches(widget.analysisId, analysis.input, context: ctx);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF0F0F1A),
        title: const Text('Nichos & Sub-nichos',
            ...
            icon: _running
                ? const SizedBox(
                    ...
                    child: CircularProgressIndicator(
                        color: Color(0xFF4D96FF), strokeWidth: 2),
                  )
          ...
          Expanded(
            child: asyncList.when(
              loading: () => const Center(
                  child: CircularProgressIndicator(color: Color(0xFF4D96FF))),
              error: (e, _) => Center(
                  child: Text('Erro: $e',
              ...
              data: (niches) => niches.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) =>
                          _NicheCard(niche: niches[i], rank: i + 1),
                    ),
            ),
          ),
        ],
        ...
        children: [
          ...
          Row(
            children: [
              _MiniBar(
                  label: 'Potencial',
                  value: niche.potentialScore,
                  color: const Color(0xFF6BCB77)),
              const SizedBox(width: 8),
              _MiniBar(
                  label: 'Crescimento',
                  value: niche.growthScore,
          ...
          Row(
            children: [
              _MiniBar(
                  label: 'Monetização',
                  value: niche.monetizationScore,
                  color: const Color(0xFFFFD93D)),
              const SizedBox(width: 8),
              _MiniBar(
                  label: 'Tendência',
                  value: niche.trendScore,
