...
serve(async (req) => {
  ...
  try {
    ...
    const quota = await reserveQuota(req, undefined, idempotencyKey, 'content-cluster');
    ...
    if (quotaReserved) await refundQuota(req, undefined, idempotencyKey, 'content-cluster');
