...
export async function handler(
  ...
  // decides whether this caller may use 'gap-analysis'. Runs after authentication
  // and before any quota reservation or AI call. Fails closed.
  const access = await requireModuleAccess(req, authUser, 'gap-analysis', corsHeaders, subjectSource);
  ...
  try {
    ...
    const quota = await reserveQuota(req, quotaClient, idempotencyKey, 'gap-analysis');
