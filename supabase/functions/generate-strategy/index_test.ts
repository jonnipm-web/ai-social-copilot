/**
 * Testes de auth + cota para generate-strategy Edge Function —
...
Deno.test('GS-4: cota esgotada -> 429, Groq nunca chamado', async () => {
  groqCalled = false;
...
Deno.test('GS-5: Groq falha depois da cota reservada -> devolve a unidade', async () => {
  groqShouldFail = true;
  quotaRefundCalls = 0;
  try {
    const res = await handler(req(BODY), validUserClient, fakeQuotaClient);
