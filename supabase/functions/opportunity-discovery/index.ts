...
serve(async (req) => {
  ...
  try {
    ...
    const quota = await reserveQuota(req, undefined, idempotencyKey, 'opportunity-discovery');
    ...
    if (quotaReserved) await refundQuota(req, undefined, idempotencyKey, 'opportunity-discovery');
