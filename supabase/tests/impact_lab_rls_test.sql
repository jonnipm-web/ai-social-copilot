...
SELECT pg_temp.expect_eq('V03 RLS enabled on EVERY impact table (8 from I1 + impact_registry_conflicts from I2 + impact_artifacts, impact_evidence_candidates from I3)',
...
SELECT pg_temp.expect_eq('V03b impact table count (a new table must be added to this suite deliberately)',
  (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
   WHERE n.nspname = 'public' AND c.relname LIKE 'impact\_%' AND c.relkind = 'r'), 11);
