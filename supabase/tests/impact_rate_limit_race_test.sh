...
# The window is a fixed 60 s: start well inside one so both sessions share it.
while [ "$(date -u +%S | sed 's/^0//')" -gt 45 ]; do sleep 1; done
...
PGAPPNAME=impact-rl-holder "${P[@]}" -c "BEGIN; $as_user SELECT hit_count FROM public.impact_rate_limit_hit('dossier_export'); SELECT pg_sleep(3); COMMIT;" >/dev/null 2>&1 &
holder=$!
tries=0
...
second="$("${P[@]}" -tA -c "BEGIN; $as_user SELECT 'COUNT=' || hit_count FROM public.impact_rate_limit_hit('dossier_export'); COMMIT;" | grep '^COUNT=' || true)"
wait "$holder"
