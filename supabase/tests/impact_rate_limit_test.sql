...
SELECT pg_temp.expect_eq('R5-01 first hit in a window counts 1', (SELECT hit_count::text FROM public.impact_rate_limit_hit('dossier_build', 60)), '1');
SELECT pg_temp.expect_eq('R5-02 hits accumulate atomically in the same window', (SELECT hit_count::text FROM public.impact_rate_limit_hit('dossier_build', 60)), '2');
SELECT pg_temp.expect_eq('R5-03 buckets are independent', (SELECT hit_count::text FROM public.impact_rate_limit_hit('dossier_export', 60)), '1');
SELECT pg_temp.expect_eq('R5-04 window start is aligned to the window size',
  (SELECT (extract(epoch FROM window_start)::bigint % 60)::text FROM public.impact_rate_limit_hit('dossier_verify', 60)), '0');
SELECT pg_temp.expect_fail('R5-05 unknown bucket refused', $$SELECT public.impact_rate_limit_hit('publish', 60)$$, 'IMPACT_RATE_LIMIT_INVALID');
SELECT pg_temp.expect_fail('R5-06 window outside 10..3600 refused', $$SELECT public.impact_rate_limit_hit('dossier_build', 1)$$, 'IMPACT_RATE_LIMIT_INVALID');
...
SELECT pg_temp.expect_eq('R5-12 another user has an isolated budget', (SELECT hit_count::text FROM public.impact_rate_limit_hit('dossier_build', 60)), '1');
RESET ROLE;

SET ROLE anon;
SELECT pg_temp.act_as(NULL);
SELECT pg_temp.expect_fail('R5-13 anon cannot call the limiter', $$SELECT public.impact_rate_limit_hit('dossier_build', 60)$$, '42501');
...
SELECT pg_temp.expect_fail('R5-15 service_role gets no execute on the limiter (no silent bypass, no expansion)',
  $$SELECT public.impact_rate_limit_hit('dossier_build', 60)$$, '42501');
...
SELECT pg_temp.expect_eq('R5-17 the limiter is SECURITY DEFINER with an empty search_path (identity = auth.uid())',
  (SELECT prosecdef::text || '/' || array_to_string(proconfig, ',') FROM pg_proc WHERE proname = 'impact_rate_limit_hit' AND pronamespace = 'public'::regnamespace),
  'true/search_path=""');
