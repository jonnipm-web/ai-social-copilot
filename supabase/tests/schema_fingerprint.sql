...
\echo '=== FUNCTIONS ==='
SELECT p.proname || '|' || l.lanname || '|' || r.rolname || '|' || (CASE WHEN p.prosecdef THEN 'DEFINER' ELSE 'INVOKER' END) || '|' || p.provolatile || '|' || COALESCE(array_to_string(p.proconfig, ','), '<none>') || '|' || pg_get_function_result(p.oid)
FROM pg_proc p
...
WHERE NOT tgisinternal
  AND tgrelid::regnamespace::text IN ('public','auth')
  AND tgrelid::regclass::text !~ '^(auth\.)?(schema_migrations|audit_log_entries|refresh_tokens|sessions|mfa_)'
...
SELECT COALESCE(r.rolname, '<none>') || '|' || d.defaclnamespace::regnamespace::text || '|' || d.defaclobjtype || '|' || d.defaclacl::text
