...
void main() {
  ...
  group('IveAvatarController', () {
    ...
    test('controller is safe after dispose', () {
      final c = IveAvatarController();
      c.dispose();
      // Should not throw after dispose (no-op)
      expect(() => c.applyVisualState(IveVisualState.success), returnsNormally);
    });
  });
  ...
  group('IveAvatar', () {
    ...
    testWidgets('has semantics label for screen readers', (tester) async {
      ...
      await tester.pump();
      final semantics = tester.getSemantics(find.byType(IveAvatar));
      expect(semantics.label, contains('IVE, assistente executiva'));
