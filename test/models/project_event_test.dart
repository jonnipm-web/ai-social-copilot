    ...
    group('event type mapping', () {
      test('all types serialize to snake_case DB values', () {
        ...
        for (final entry in expected.entries) {
          final event = ProjectEvent(
            id: '', projectId: '', userId: '',
            eventType: entry.key, title: '', description: '',
            createdAt: DateTime.now(),
          );
      ...
      test('fromJson round-trips all type strings', () {
        final types = [
          'project_created', 'stage_changed', 'analysis_completed',
          'decision_taken', 'opportunity_created', 'document_added',
          'action_completed', 'check_in', 'interview_completed',
          'relationship_detected', 'health_changed', 'priority_changed',
        ];

        for (final t in types) {
          final json = {
            'id': '', 'project_id': '', 'user_id': '',
            'event_type': t, 'title': '', 'description': '',
      ...
      test('unknown type defaults to checkIn', () {
        final json = {
          'id': '', 'project_id': '', 'user_id': '',
          'event_type': 'unknown_event_type', 'title': '', 'description': '',
          'metadata': <String, dynamic>{},
          'created_at': DateTime.now().toIso8601String(),
        };
        final event = ProjectEvent.fromJson(json);
    ...
    group('filterGroup getter', () {
      test('analysis types map to Análises', () {
        for (final t in [
          ...
          final e = ProjectEvent(
            id: '', projectId: '', userId: '', eventType: t,
      ...
      test('knowledge types map to Conhecimento', () {
        for (final t in [
          ...
          expect(e.filterGroup, 'Conhecimento', reason: '$t should be Conhecimento');
        }
      });
